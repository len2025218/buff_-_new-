...
namespace rm_buff
{
    ...
    static constexpr float BBOX_CONF_THRESH = 0.34;
    static constexpr float MERGE_CONF_ERROR = 0.15;
    static constexpr float MERGE_MIN_IOU = 0.3;
    ...
    {
        //  size_t test_capacity = 3000; // 容量大小
        ...
        for (auto stride : strides)
        {
            ...
            for (int g1 = 0; g1 < num_grid_h; g1++)
            {
                for (int g0 = 0; g0 < num_grid_w; g0++)
                {
                    ...
                    // cout<<"generate_grids_and_stride2.5"<<endl;
                                       ...
                                       std::vector<BuffObject> &objects)
    {
        ...
        for (int anchor_idx = 0; anchor_idx < num_anchors; anchor_idx++)
        {
            ...
            // float *cls_scores = feat_ptr + basic_pos + 11 + NUM_COLORS;
            // float cls_prob0 = cls_scores[0]; // 待击概率
            // float cls_prob1 = cls_scores[1]; // 已击概率

            float class_prob0 = feat_ptr[basic_pos + 11 + NUM_COLORS];

            float class_prob1 = feat_ptr[basic_pos + 11 + NUM_COLORS + 1];
            // std::cout << "class_prob0" << class_prob0 << std::endl;
            // std::cout << "class_prob1" << class_prob1 << std::endl;
            ...
            if (box_prob >= prob_threshold)
            {
                ...
                // 存储双类别概率
                obj.cls_prob[0] = class_prob0;
                obj.cls_prob[1] = class_prob1;
                obj.cls = (class_prob0 > class_prob1) ? 0 : 1;
                ...
                for (int i = 0; i < 5; i++)
                {
                    ...
                    // size_t obj_pts_size = sizeof(obj.pts);
                }
    ...
    bool Detector::initModel(std::string path)
    {
    ...
    bool Detector::detect(cv::Mat &src, std::vector<BuffObject> &objects)
    {
        if (src.empty())
        {
            std::cout << "[DETECT] ERROR: 传入了空的src" << std::endl;
            ;
            return false;
        }
        // cout << 123 << endl;
        ...
        // Get input tensor by index获取推理请求对象中的输入张量
        ...
        // Step 7: 目标筛选逻辑（新增核心部分）
        std::vector<BuffObject> final_objects;
        std::vector<BuffObject> pending_targets; // 待击目标暂存
        // std::vector<BuffObject> pending_targets_2;
        // 分离目标类型
        for (auto &obj : objects)
        {
            if (obj.cls == 0) // 待击目标
            {
                pending_targets.push_back(obj);
            }
            else // 已击目标直接保留
            {
        ...
        // 待击目标筛选策略
        if (pending_targets.size() > 1)
        {
            // 按已击概率升序排序（概率越小越优先）
            std::sort(pending_targets.begin(), pending_targets.end(),
                      [](const BuffObject &a, const BuffObject &b)
                      {
                          return a.cls_prob[0] > b.cls_prob[0]; // 比较已击概率
                      });

            // for (size_t i = 1; i < pending_targets.size(); ++i)
            // { // 注意使用 size_t
            //     const auto &target = pending_targets[i];
            //     // 绘制五边形（连接5个顶点）
            //     for (int j = 0; j < 5; ++j)
            //     {
            //         cv::line(
            //             src,
            //             target.apex[j % 5],       // 当前顶点
            //             target.apex[(j + 1) % 5], // 下一个顶点
            //             cv::Scalar(255, 0, 0),    // 绿色表示其他待击目标
            //             2,                        // 线宽
            //             cv::LINE_AA);
            //     }
            // }
            // 保留已击概率最小的目标
            final_objects.push_back(pending_targets[0]);

            // // 调试信息
            // std::cout << "[筛选] 待击目标数: " << pending_targets.size()
            //           << " | 最小已击概率: " << pending_targets[0].cls_prob[1]
            //           << std::endl;
        }
        else if (pending_targets.size() == 1)
        {
            final_objects.push_back(pending_targets[0]);
        }

        // 更新最终结果
        objects = std::move(final_objects);
